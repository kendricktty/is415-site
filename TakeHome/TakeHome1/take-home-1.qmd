...
The EPSG area code for Myanmar is [`4239`](https://epsg.io/4239). Before we work further with our datasets, we will need to transform the coordinate system to use it.
...
multiplicity(acled_mya_ppp)
sum(multiplicity(acled_mya_ppp) > 1)
```

#### TH1.3.4.4 Combining point events object and 

## TH1.4 Interlude: A brief history of Myanmar's ethnic groups

## TH1.5 First Order SPPA 
...
```{r}
tm_shape(mmr_admin1) + tm_polygons() + tm_shape(acled_mya_sf) + tm_dots(size = 0.1) + tm_facets(by="Month_num", free.coords=FALSE, drop.units=TRUE, ncol=3, nrow=1)
```