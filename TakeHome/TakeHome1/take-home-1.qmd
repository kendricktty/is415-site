...
for (coupStatus in beforeAfterCoup) {
  ...
  acled_ppp_beforeAfterCoup_list[[as.character(quarter)]] <- acled_mya_beforeCoup_ppp
...
selected_quarters = c("2023-4Q", "2021-1Q", "2020-2Q", "2019-1Q")
...
## TH1.6 First-order SPPA: Nearest Neighbour Analysis
...
for (coupStatus in beforeAfterCoup) {
  ...
  ppp_item <- acled_ppp_beforeAfterCoup_list[[as.character(coupStatus)]]
  clarkevans.test(ppp_item, correction = "none", clipregion = "mmr_admin1_owin", alternative = c("clustered"), nsim = 99)
}
```
...
for (coupStatus in beforeAfterCoup) {
  ...
  plot(F_function, main = paste("F function ", coupStatus, " coup"))
  
  # Perform complete spatial randomness test
  csr <- envelope(ppp_item, Fest, nsim=999)
  plot(csr, main = paste("Complete spatial randomness test for F function ", coupStatus, " coup"))
}
```
...
for (coupStatus in beforeAfterCoup) {
  ...
  plot(G_function, xlim=c(0, 500), main = paste("G function ", coupStatus, " coup"))
  
  # Perform complete spatial randomness test
  csr <- envelope(ppp_item, Gest, nsim=999)
  plot(csr, main = paste("Complete spatial randomness test for G function ", coupStatus, " coup"))
}
```
...
for (coupStatus in beforeAfterCoup) {
  ...
  plot(K_function, . -r ~ r, ylab= "K(d)-r", xlab = "d(m)", main = paste("G function ", coupStatus, " coup"))
  
  # Perform complete spatial randomness test
  csr <- envelope(ppp_item, Kest, nsim=999, rank=1, glocal=TRUE)
  plot(csr, . - r ~ r, xlab="d", ylab="K(d)-r", main = paste("Complete spatial randomness test for K function ", coupStatus, " coup"))
}
```
...
for (coupStatus in beforeAfterCoup) {
  ...
  plot(L_function, . -r ~ r, ylab= "L(d)-r", xlab = "d(m)", main = paste("G function ", coupStatus, " coup"))
  
  # Perform complete spatial randomness test
  csr <- envelope(ppp_item, Kest, nsim=999, rank=1, glocal=TRUE)
  plot(csr, . - r ~ r, xlab="d", ylab="L(d)-r", main = paste("Complete spatial randomness test for L function ", coupStatus, " coup"))
}
```

## TH1.8 First-order STPPA: Spatio-temporal KDE

## TH1.9 Second-order STPPA
