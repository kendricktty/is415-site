...
ggplot(acled_mya_civilianTargeting_by_quarter, aes(x=Quarter_num, y=Percentage_incidents_civilians_targeting)) +     
  ...
  geom_line(color = "skyblue1") + geom_point() +
    labs(title = "Incidents involving civilian targeting in Myanmar by Quarter", subtitle = "1 = Q1 2021, 15 = Q3 2024",
         x = "Quarter",
         y = "Incidents with civilian targeting") +
    theme_minimal()
```

The percentage of violent conflict incidents targeting civilians trended upwards throughout the study duration.
...
Ripley's K-function valuates the distribution of points over a range of distances, giving insight into spatial clustering or dispersion at different scales. In contrast, the **F-function** (nearest neighbor distribution) and **G-function** (empty space function) are limited to analyzing specific distances or single-scale proximity between points. This makes Ripley's K-function a more comprehensive analysis of spatial point patterns at multiple spatial scales, but more computationally complex.

### TH1.8.1 Extracting study period

The K- and L-functions run slower when fed with more data. Hence, we need to extract a smaller study period. Since the year 2022 saw more fatalities, it makes sense to extract only the events that occurred in 2022.
...
plot(F_function, main = paste("F-function estimation for", study_name))

F_csr <- envelope(ppp, Fest, nsim = 99)
plot(F_csr, main = paste("F function test for spatial randomness for", study_name))
```

#### TH1.8.1.3 K-function

```{r}
K_function <- Kest(study_ppps_2022[[study_name]], correction = "Ripley")
plot(K_function, main = paste("K-function estimation for", study_name), . -r ~ r, ylab= "K(d)-r", xlab = "d(m)")

K_csr <- envelope(ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
plot(K_csr, main = paste("K function test for spatial randomness for", study_name), . - r ~ r, xlab="d", ylab="K(d)-r")
```

#### TH1.8.1.4 L-function

```{r}
L_function <- Lest(study_ppps_2022[[study_name]], correction = "Ripley")
plot(L_function, main = paste("L-function estimation for", study_name), . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")

L_csr <- envelope(ppp, Lest, nsim = 99, rank = 1, glocal=TRUE)
plot(L_csr, main = paste("L function test for spatial randomness for", study_name), . - r ~ r, xlab="d", ylab="L(d)-r")
```
...
```{r}
study_name = "Shan"
G_function <- Gest(study_ppps_original[[study_name]], correction = "border")
plot(G_function, xlim=c(0,500), main = paste("G-function estimation for", study_name))
...
```{r}
F_function <- Fest(study_ppps_original[[study_name]])
plot(F_function, main = paste("F-function estimation for", study_name))
...
#### TH1.8.2.3 K-function

```{r}
K_function <- Kest(study_ppps_original[[study_name]], correction = "Ripley")
plot(K_function, main = paste("K-function estimation for", study_name), . -r ~ r, ylab= "K(d)-r", xlab = "d(m)")
...
#### TH1.8.2.4 L-function

```{r}
L_function <- Lest(study_ppps_original[[study_name]], correction = "Ripley")
plot(L_function, main = paste("L-function estimation for", study_name), . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")
...
6.  Smith, M. (1999). Burma: Insurgency and the politics of ethnicity (2. (updated) ed). Univ. Press \[u.a.\].
7.  Sullivan, D. P. (2021, October 21). Dire consequences: Addressing the humanitarian fallout from myanmar’s coup. Refugees International; Refugees International. <https://www.refugeesinternational.org/reports-briefs/dire-consequences-addressing-the-humanitarian-fallout-from-myanmars-coup/>
8.  UNDP (Ed.). (2024). Breaking the gridlock: Reimagining cooperation in a polarized world. United Nations Development Programme (UNDP).
