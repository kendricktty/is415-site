...
for (quarter in quarters) {
  ...
  acled_ppp_list[[as.character(quarter)]]
...
kde <- density(acled_mya_ppp.km, sigma=bw.diggle, edge=TRUE, kernel="gaussian")
plot(kde)

# Create a list to store KDE results and track quarters for each quarter
kde_results <- list()

for (i in length(quarters)) {
  ppp_item <- acled_ppp_list[i]
  ppp_item
  # Compute KDE and append to list
  kde_results[[as.character(quarters[i])]] <- density(ppp_item, sigma=0.6, edge=TRUE, kernel="gaussian")
}
