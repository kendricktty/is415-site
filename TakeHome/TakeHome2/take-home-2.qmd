...
- If the key indicators of drug abuse and trafficking in **[Thailand](https://en.wikipedia.org/wiki/Thailand)** are independent from space,
...
We are particularly interested in drug use and drug trafficking cases, so we will now attempt to plot the changes in the number of each type of case between 2017 and 2022.

### TH2.4.1 Plotting changes in drug trafficking cases by time

```{r}
thailand_drug_trafficking <- thailand_drug %>% filter(types_of_drug_offenses == 'trafficking_cases' & types_of_drug_offenses == 'suspects_in_trafficking_cases') %>%
  group_by(types_of_drug_offenses, fiscal_year) %>%
  summarise(total_cases = sum(no_cases)) %>%
  summarise(total_suspected_cases = sum(no_cases)) %>%
  ungroup()
thailand_drug_trafficking

ggplot() +     
  # geom_bar(stat = "identity", fill = "red4") +
  geom_line(color = "purple4", data = thailand_drug_trafficking, aes(x = fiscal_year, y = total_cases)) + geom_point() +
    labs(title = "Drug trafficking and suspected cases by year", subtitle = "1 = Q1 2021, 6 = Q2 2022, 11 = Q3 2023, 15 = Q3 2024",
         x = "Year",
         y = "Number of cases") +
    theme_minimal()
```
