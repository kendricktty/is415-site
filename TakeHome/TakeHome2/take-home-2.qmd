...
```{r}
thailand_drug_analysis <- thailand_drug %>% group_by(types_of_drug_offenses) %>% summarise(total_cases = sum(no_cases)) %>% ungroup()
...
```{r}
thailand_drug_trafficking <- thailand_drug %>% filter(types_of_drug_offenses == 'trafficking_cases') %>%
...
ggplot() +     
  ...
  geom_line(color = "skyblue1", data = thailand_drug_trafficking, aes(x = fiscal_year, y = total_cases)) + geom_point() +
