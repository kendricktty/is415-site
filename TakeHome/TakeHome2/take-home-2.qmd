...
### TH2.1.1 Provincial map of Thailand

[Map of the 77 provinces of Thailand.](https://upload.wikimedia.org/wikipedia/commons/thumb/c/c5/Thailand_provinces_en.svg/1164px-Thailand_provinces_en.svg.png)
...
tmap_arrange(use_map + tm_layout(main.title = "Drug use across Thailand (quantile)", title.size = 0.05), use_map_equal + tm_layout(main.title = "Drug use across Thailand (equal)", title.size = 0.25), asp = 1, ncol = 2)
```
...
tmap_arrange(trafficking_map  + tm_layout(main.title = "Drug trafficking across Thailand (quantile)", title.size = 0.05), trafficking_map_equal  + tm_layout(main.title = "Drug trafficking across Thailand (equal)"), asp = 1, ncol = 2)
```
...
is_spacetime_cube(thailand_esa_use_spt)
```
...
thailand_ehsa_use <- thailand %>% left_join(thailand_ehsa_use, by = join_by(ADM1_EN == ADM1_EN))
thailand_ehsa_tr <- thailand %>% left_join(thailand_ehsa_tr, by = join_by(ADM1_EN == ADM1_EN))
```
