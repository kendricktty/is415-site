...
## TH2.4.3 Grouping observations by case type

The following code chunks create a filtered table for the drug use and trafficking observations, groups and summarises them by province, and plots the numbers onto the provincial map of Thailand.

```{r}
thailand_2021_drug <- thailand_drug %>%
  filter(fiscal_year %in% c("2020", "2021")) %>%
  filter(types_of_drug_offenses %in% c("drug_use_cases", "trafficking_cases", "possession_cases", "possession_with_intent_to_distribute_cases")) %>%
  group_by(province_en) %>%
  summarise(total_use_cases = sum(no_cases[types_of_drug_offenses %in% c("drug_use_cases", "possession_cases")]),
            total_trafficking_cases = sum(no_cases[types_of_drug_offenses %in% c("trafficking_cases", "possession_with_intent_to_distribute_cases")])) %>%
  ungroup()
...
```
This is a surprising result for our `drug_use` data, with most provinces on the map showing local Moran's I values that are statistically insignificant.
...
```{r}
local_MI_use <- localmoran_perm(thailand_2021$total_use_cases, rswm_q_use, nsim = 9999)
local_MI_trafficking <- localmoran_perm(thailand_2021$total_trafficking_cases, rswm_q_tr, nsim = 9999)
...
```{r}
use.localMI.map <- tm_shape(thailand_2021_use.localMI) + tm_fill(col = "Ii", style = "pretty", title = "drug_use LM permutation") + tm_borders(alpha = 0.5)
use.pv.map <- tm_shape(thailand_2021_use.localMI) + tm_fill(col = "Pr.Ii", 
          breaks=c(-Inf, 0.001, 0.01, 0.05, 0.1, Inf),
          palette=c("#b7dce9","#c9e3d2","#f5f3a6","#ec9a64","#d21b1c"), 
          title = "drug_use p-values") +
...
```{r}
tr.localMI.map <- tm_shape(thailand_2021_trafficking.localMI) + tm_fill(col = "Ii", style = "pretty", title = "drug_trafficking LM permutation") + tm_borders(alpha = 0.5)
tr.pv.map <- tm_shape(thailand_2021_trafficking.localMI) + tm_fill(col = "Pr.Ii", 
          breaks=c(-Inf, 0.001, 0.01, 0.05, 0.1, Inf),
          palette=c("#b7dce9","#c9e3d2","#f5f3a6","#ec9a64","#d21b1c"), 
          title = "drug_trafficking p-values") +
...
3. Kam, T. S. (2022). R for Geospatial Data Science and Analytics. https://r4gdsa.netlify.app
4. Khant, M. N. (2024, February 10). Take-Home Exercise 02: Spatio-Temporal Analysis of Dengue Fever Outbreak in Tainan City, Taiwan. https://is415-geospatial-with-khant.netlify.app/take-home_ex/take-home_ex02/take-home_ex02
5. Kongjareon, Y., Samoh, N., Peerawaranun, P. et al. Pride-based violence, intoxicated sex and poly-drug use: a vocational school-based study of heterosexual and LGBT students in Bangkok. BMC Psychiatry 22, 148 (2022). https://doi.org/10.1186/s12888-022-03777-7
6. Kanato, M., Sarasiri, R., & Leyatikul, P. (Eds.). (2022). ASEAN drug monitoring report 2021. ASEAN Narcotics Cooperation Center.
7. Moraga, P. (2024). Spatial neighbourhood matrices. In Spatial statistics for Data Science: Theory and practice with R (pp. 83–94). CRC Press.
8. Thailand. (n.d.). Hands-Off Cain. Retrieved 23 September 2024, from https://www.handsoffcain.info/bancadati/asia-middle-east-australia-and-oceania/thailand-17000464
9. Tobler, W. R. (1970). A computer movie simulating urban growth in the Detroit region. Economic Geography (Supplement: Proceedings, International Geographical Union. Commission on Quantitative Methods), 46, 234-240. https://doi.org/10.2307/143141
10. United Nations Development Programme. (2024, March 13). Human Development Report 2023/2024 (PDF). Retrieved September 23, 2024, from https://hdr.undp.org/sites/default/files/reports/3503/hdr2023-24reporten.pdf
11. World Bank. (n.d.). Gini index. Retrieved September 23, 2024, from https://www.worldbank.org/en/topic/poverty/brief/gini-index
12. World Bank Open Data. (n.d.). Unemployment, total (% of total labor force) (national estimate) - Thailand. Retrieved 27 September 2024, from https://data.worldbank.org/indicator/SL.UEM.TOTL.NE.ZS?end=2023&locations=TH
13. World Drug Day report highlights spike in drug use, increased trafficking | UN News. (2024, June 26). United Nations. https://news.un.org/en/story/2024/06/1151446biggest increase from 1873kg in 2020 to 4520kg in 2021 (Kanato et al., 2022).