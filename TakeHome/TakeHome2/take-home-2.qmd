...
A non-parametric test, the Man-Kendall trend test is used to assess the direction (increasing/decreasing) of a set of data values over time, and whether the trend in either direction is statistically significant. It returns two values: Kendall's Score `s`, and Kendall's Tau `tau`, which is a correlation coefficient is positive ($tau$ > 0) if a correlation is present, and zero ($tau$ = 0) otherwise (Khant, 2024) (Gilbert, 1987).
...
```{r}
thailand_gi_stars_per_year <- thailand_drug_nb %>%
  group_by(fiscal_year) %>%
  ...
  ungroup(gi_star_use) %>% ungroup(gi_star_tr)
...
```{r}
# Surin province
cbg_surin <- thailand_gi_stars_per_year %>% ungroup() %>%
  filter(ADM1_EN == "Surin") %>%
  select(ADM1_EN, fiscal_year, gi_star_use, gi_star_tr)
# Kanchanaburi province
cbg_kanchanaburi <- thailand_gi_stars_per_year %>% ungroup() %>%
...
ggplot() + geom_line(
  data = cbg_surin,
  mapping = aes(x = fiscal_year, y = gi_star_use$gi_star, color = "Drug Use in Surin Province")
) + 
  geom_line(
    data = cbg_surin,
    mapping = aes(x = fiscal_year, y = gi_star_tr$gi_star, color = "Drug Trafficking in Surin Province")
) +
...
```{r}
cbg_surin_use <- cbg_surin %>% summarise(
  mann_kendall = list(unclass(Kendall::MannKendall(gi_star_use)))) %>%
  tidyr::unnest_wider(mann_kendall_use)

cbg_surin_tr <- cbg_surin %>% summarise(
  mann_kendall = list(unclass(Kendall::MannKendall(gi_star_tr)))) %>%
  tidyr::unnest_wider(mann_kendall_tr)
```
...
```{r}
ggplot() + geom_line(
  data = cbg_kanchanaburi,
  mapping = aes(x = fiscal_year, y = gi_star_use$gi_star, color = "Drug Use in Kanchanaburi Province")
) + 
  geom_line(
    data = cbg_kanchanaburi,
    mapping = aes(x = fiscal_year, y = gi_star_tr$gi_star, color = "Drug Trafficking in Kanchanaburi Province")
) +
...
12. Tobler, W. R. (1970). A computer movie simulating urban growth in the Detroit region. Economic Geography (Supplement: Proceedings, International Geographical Union. Commission on Quantitative Methods), 46, 234-240. https://doi.org/10.2307/143141
13. United Nations Development Programme. (2024, March 13). Human Development Report 2023/2024 (PDF). Retrieved September 23, 2024, from https://hdr.undp.org/sites/default/files/reports/3503/hdr2023-24reporten.pdf
14. World Bank. (n.d.). Gini index. Retrieved September 23, 2024, from https://www.worldbank.org/en/topic/poverty/brief/gini-index
15. World Bank Open Data. (n.d.). Unemployment, total (% of total labor force) (national estimate) - Thailand. Retrieved 27 September 2024, from https://data.worldbank.org/indicator/SL.UEM.TOTL.NE.ZS?end=2023&locations=TH
16. World Drug Day report highlights spike in drug use, increased trafficking | UN News. (2024, June 26). United Nations. https://news.un.org/en/story/2024/06/1151446biggest increase from 1873kg in 2020 to 4520kg in 2021 (Kanato et al., 2022).