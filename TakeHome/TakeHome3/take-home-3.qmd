...
jb_kulai_bounded$density <- lengths(intersections)
```

The following function allows us to manually aggregate the average property price within each hexagonal grid.

```{r}
avg_prices <- numeric(nrow(jb_kulai_bounded))

for (i in seq_len(nrow(jb_kulai_bounded))) {
  points_in_hex <- property[st_within(property, jb_kulai_bounded[i, ], sparse = FALSE), ]
...
jb_kulai_bounded$avg_price_inefficient <- avg_prices
```

```{r}
# Extract maximum property price for each cell
avg_prices <- sapply(intersections, function(idx) {
  if (length(idx) == 0) return(0)
  mean(property$`Transaction Price`[idx], na.rm = TRUE)
})
jb_kulai_bounded$avg_price <- avg_prices
